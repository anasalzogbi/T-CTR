...
void c_ctr::stochastic_learn_map_estimate(const c_data* users, const c_data* items,
                               const c_double_data* users_a, const c_double_data* items_a, 
...
void c_ctr::learn_map_estimate(const c_data* users, const c_data* items,
  ...
  while ((iter < param->max_iter and converge > 1e-4 ) or iter < min_iter) {
    ...
    for (i = 0; i < m_num_users; i ++) {
      ...
      // Get the list of confidence values for this user, one value for each rated item
      user_confs = users_a->m_vec_data[i];
      ...
      if (n > 0) {
        ...
        for (l=0; l < n; l ++) {
          ...
          a = user_confs[l];
          gsl_vector_const_view v = gsl_matrix_const_row(m_V, j); 
          gsl_blas_dger(a - param->b, &v.vector, &v.vector, A); 
          gsl_blas_daxpy(a, &v.vector, x);
        }
    ...
    for (j = 0; j < m_num_items; j ++) {
      ...
      // Get the list of confidence values for this item, one value for each rating
      item_confs = items_a->m_vec_data[j];
      ...
      if (m>0) {
        ...
        for (l = 0; l < m; l ++) {
          ...
          a = item_confs[l];
          gsl_vector_const_view u = gsl_matrix_const_row(m_U, i);  
          gsl_blas_dger(a - param->b, &u.vector, &u.vector, A);
          gsl_blas_daxpy(a, &u.vector, x);
        }
        ...
        // If a is the same for all ratings: 
