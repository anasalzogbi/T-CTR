...
c_double_data::c_double_data() {
}

c_double_data::~c_double_data() {
  for (size_t i = 0; i < m_vec_data.size(); i ++) {
    double* confs = m_vec_data[i];
    if (confs != NULL) delete [] confs;
  }
  m_vec_data.clear();
  m_vec_len.clear();
}

void c_double_data::read_data(const char * data_filename,  int OFFSET) {
  ...
  while ((fscanf(fileptr, "%10d", &length) != EOF)) {
    double * confs = NULL;
    if (length > 0) {
      confs = new double[length];
      for (n = 0; n < length; n++) {
        fscanf(fileptr, "%lf", &conf);
        confs[n] = conf;
      }
    }
    m_vec_data.push_back(confs);
