...
void print_usage_and_exit() {
  ...
  printf("      --user_a:         user confidence file, required\n");
  printf("      --item:           item file, required\n");
  printf("      --item_a:         item confidence file, required\n");
...
int main(int argc, char* argv[]) {
  ...
  const char* const short_options = "hd:x:c:i:f:a:b:u:v:r:s:m:k:t:e:y:z:w:";
  const struct option long_options[] = {
    {"help",          no_argument,       NULL, 'h'},
    {"directory",     required_argument, NULL, 'd'},
    {"user",          required_argument, NULL, 'x'},
    {"user_a",        required_argument, NULL, 'c'},
    {"item",          required_argument, NULL, 'i'},
    {"item_a",        required_argument, NULL, 'f'},
  ...
  char*  user_path = NULL;
  char*  item_path = NULL;
  char*  user_a_path = NULL;
  char*  item_a_path = NULL;
  double a = 1.0;
  double b = 0.01;
  ...
  printf("user file: %s\n", user_path);
  if (!file_exists(user_a_path)) {
    printf("user confidence file %s doesn't exist! quit ...\n", user_a_path);
    exit(-1);
  }
  printf("user confidence file: %s\n", user_a_path);
  ...
  if (!file_exists(item_a_path)) {
    printf("item confidence file %s doesn't exist! quit ...\n", item_a_path);
    exit(-1);
  }
  printf("item confidence file: %s\n", item_a_path);
  ...
  // read users confidence
  printf("reading user confidence matrix from %s ...\n", user_a_path);
  c_double_data* users_a = new c_double_data(); 
  users_a->read_data(user_a_path);
  ...
  // read items confidence
  printf("reading item confidence matrix from %s ...\n", item_a_path);
  c_double_data* items_a = new c_double_data(); 
  items_a->read_data(item_a_path);
  ...
  if (learning_rate <= 0) {
    ctr->learn_map_estimate(users, items, users_a, items_a, c, &ctr_param, directory);
  } else {
    ctr->stochastic_learn_map_estimate(users, items, users_a, items_a, c, &ctr_param, directory);
  }
